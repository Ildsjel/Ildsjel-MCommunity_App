...
export function TrackPanel({ release, onRelease }: { release: any; onRelease: (r: any) => void }) {
  ...
  const inputStyle = (border = 'rgba(216,207,184,0.15)'): React.CSSProperties => ({
    ...
    color: 'var(--ink)', fontFamily: 'var(--font-mono)', fontSize: '0.625rem',
  ...
  return (
    <Box sx={{ borderTop: '1px solid rgba(216,207,184,0.08)', mt: 0.75, pt: 0.75 }}>
      {tracks.length === 0 && !showAdd && (
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.5625rem', color: 'var(--muted)', letterSpacing: '0.1em' }}>
          No tracks yet
        </span>
      )}
              ...
              {isEditing && vals ? (
                <Box sx={{ border: '1px solid rgba(216,207,184,0.2)', borderRadius: '3px', backgroundColor: '#1a1424', p: '8px 10px', display: 'flex', flexDirection: 'column', gap: 0.625 }}>
                  <Box sx={{ display: 'flex', gap: 0.625 }}>
                    <Box sx={{ width: 44 }}>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.625rem', color: 'var(--muted)', letterSpacing: '0.08em', display: 'block', marginBottom: 2 }}>#</span>
                      <input value={vals.number} onChange={(e) => setEditing((p) => ({ ...p, [t.id]: { ...vals, number: e.target.value } }))} style={{ ...inputStyle(), width: 40 }} />
                    </Box>
                    <Box sx={{ flex: 1 }}>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.625rem', color: 'var(--muted)', letterSpacing: '0.08em', display: 'block', marginBottom: 2 }}>TITLE</span>
                      <input value={vals.title} onChange={(e) => setEditing((p) => ({ ...p, [t.id]: { ...vals, title: e.target.value } }))} style={inputStyle()} />
                    </Box>
                    <Box sx={{ width: 68 }}>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.625rem', color: 'var(--muted)', letterSpacing: '0.08em', display: 'block', marginBottom: 2 }}>DURATION</span>
                      <input value={vals.duration} onChange={(e) => setEditing((p) => ({ ...p, [t.id]: { ...vals, duration: e.target.value } }))} placeholder="4:32" style={inputStyle()} />
                    </Box>
                  </Box>
                  <Box>
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.625rem', color: 'var(--muted)', letterSpacing: '0.08em', display: 'block', marginBottom: 2 }}>LYRICS (OPTIONAL)</span>
                  ...
                  {trackErr[t.id] && <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.5625rem', color: 'var(--accent)', letterSpacing: '0.08em' }}>⚠ {trackErr[t.id]}</span>}
                  <Box sx={{ display: 'flex', gap: 0.5, justifyContent: 'flex-end' }}>
                    <Box component="button" type="button" onClick={() => cancelEdit(t.id)} sx={{ border: '1px solid rgba(216,207,184,0.18)', borderRadius: '2px', px: 0.75, height: 20, background: 'none', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: '0.625rem', color: 'var(--muted)', letterSpacing: '0.08em' }}>CANCEL</Box>
                    <Box component="button" type="button" onClick={() => handleUpdateTrack(t)} disabled={isSaving} sx={{ border: '1px solid rgba(216,207,184,0.35)', borderRadius: '2px', px: 0.75, height: 20, background: 'none', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: '0.625rem', color: 'var(--ink)', letterSpacing: '0.08em', '&:disabled': { opacity: 0.4 } }}>{isSaving ? '…' : 'SAVE'}</Box>
                  </Box>
                </Box>
              ) : (
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, px: 0.625, py: 0.375, borderRadius: '2px', '&:hover': { backgroundColor: 'rgba(216,207,184,0.03)' } }}>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.5625rem', color: 'var(--muted)', width: 18, flexShrink: 0, textAlign: 'right' }}>{t.number}.</span>
                  ...
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.5625rem', color: 'var(--muted)', flexShrink: 0 }}>{t.duration}</span>
                  {t.lyrics && <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.625rem', color: 'rgba(154,122,191,0.6)', letterSpacing: '0.08em', flexShrink: 0 }} title="Has lyrics">♪</span>}
                  <Box sx={{ display: 'flex', gap: 0.375, flexShrink: 0 }}>
                    <Box component="button" type="button" onClick={() => startEdit(t)} sx={{ border: '1px solid rgba(216,207,184,0.15)', borderRadius: '2px', px: 0.5, height: 16, background: 'none', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: '0.625rem', color: 'var(--muted)', '&:hover': { color: 'var(--ink)' } }}>✎</Box>
                    <Box component="button" type="button" onClick={() => handleDeleteTrack(t)} disabled={isSaving} sx={{ border: '1px solid rgba(196,58,42,0.2)', borderRadius: '2px', px: 0.5, height: 16, background: 'none', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: '0.625rem', color: 'var(--accent)', '&:hover': { borderColor: 'var(--accent)' }, '&:disabled': { opacity: 0.4 } }}>✕</Box>
                  </Box>
                </Box>
              )}
            </Box>
      ...
      {showAdd ? (
        <Box component="form" onSubmit={handleAddTrack} sx={{ border: '1px solid rgba(216,207,184,0.18)', borderRadius: '3px', backgroundColor: '#1a1424', p: '8px 10px', mt: 0.625, display: 'flex', flexDirection: 'column', gap: 0.625 }}>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.5625rem', color: 'var(--muted)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>New track</span>
          <Box sx={{ display: 'flex', gap: 0.625 }}>
            <Box sx={{ width: 44 }}>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.625rem', color: 'var(--muted)', letterSpacing: '0.08em', display: 'block', marginBottom: 2 }}>#</span>
          ...
          {addErr && <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.5625rem', color: 'var(--accent)' }}>⚠ {addErr}</span>}
          <Box sx={{ display: 'flex', gap: 0.5, justifyContent: 'flex-end' }}>
            <Box component="button" type="button" onClick={() => { setShowAdd(false); setAddErr(null) }} sx={{ border: '1px solid rgba(216,207,184,0.18)', borderRadius: '2px', px: 0.75, height: 20, background: 'none', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: '0.625rem', color: 'var(--muted)', letterSpacing: '0.08em' }}>CANCEL</Box>
            <Box component="button" type="submit" disabled={addSaving} sx={{ border: '1px solid rgba(216,207,184,0.35)', borderRadius: '2px', px: 0.75, height: 20, background: 'none', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: '0.625rem', color: 'var(--ink)', letterSpacing: '0.08em', '&:disabled': { opacity: 0.4 } }}>{addSaving ? '…' : '＋ ADD'}</Box>
          </Box>
        </Box>
      ) : (
        <Box component="button" type="button" onClick={() => { setShowAdd(true); setAddForm((p) => ({ ...p, number: String(nextNumber) })) }}
          sx={{ mt: 0.5, border: '1px dashed rgba(216,207,184,0.15)', borderRadius: '2px', px: 0.875, height: 22, background: 'none', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: '0.625rem', letterSpacing: '0.1em', color: 'var(--muted)', '&:hover': { borderColor: 'rgba(216,207,184,0.3)', color: 'var(--ink)' }, display: 'flex', alignItems: 'center', gap: 0.5 }}>
          ＋ ADD TRACK
        </Box>
      )}
    </Box>

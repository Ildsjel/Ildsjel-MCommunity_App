  ...
  return (
    <Box sx={{ maxWidth: 480, mx: 'auto', px: 2, pt: 2, pb: 10 }}>
      ...
      <Box sx={{ border: '1.5px solid rgba(216,207,184,0.2)', borderRadius: '3px', backgroundColor: '#120e18', p: 1.5, mb: 2 }}>
        <span style={{ ...lbl, fontSize: '0.625rem', display: 'block', marginBottom: 12 }}>GENERATE TOKEN</span>
        <Box component="form" onSubmit={handleGenerate} sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          ...
          <Box component="button" type="submit" disabled={generating} sx={{ border: '1.5px solid rgba(216,207,184,0.3)', borderRadius: '3px', py: 0.875, background: 'none', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: '0.625rem', letterSpacing: '0.12em', color: 'var(--ink)', '&:disabled': { opacity: 0.4 } }}>
      ...
      {newToken && (
        <Box sx={{ border: '1.5px solid rgba(106,154,122,0.4)', borderRadius: '3px', backgroundColor: 'rgba(106,154,122,0.08)', p: 1.5, mb: 2 }}>
          <span style={{ ...lbl, fontSize: '0.5625rem', color: '#6a9a7a', display: 'block', marginBottom: 8 }}>✓ TOKEN GENERATED — COPY NOW</span>
          <Box sx={{ display: 'flex', gap: 0.75, alignItems: 'center' }}>
            <Typography sx={{ fontFamily: '"JetBrains Mono", monospace', fontSize: '0.6875rem', color: 'var(--ink)', wordBreak: 'break-all', flex: 1 }}>
              {newToken}
            </Typography>
            <Box component="button" onClick={handleCopy} sx={{ border: '1px solid rgba(216,207,184,0.3)', borderRadius: '2px', px: 0.875, height: 28, background: 'none', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: '0.5625rem', color: copied ? '#6a9a7a' : 'var(--muted)', flexShrink: 0 }}>
              {copied ? 'COPIED' : 'COPY'}
            </Box>
          </Box>
        </Box>
      ...
      {!loading && tokens.length > 0 && (
        <Box>
          <span style={{ ...lbl, fontSize: '0.625rem', display: 'block', marginBottom: 8 }}>ISSUED TOKENS</span>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
            {tokens.map((token) => (
              <Box key={token.id} sx={{ border: '1px solid rgba(216,207,184,0.15)', borderRadius: '3px', backgroundColor: '#120e18', px: 1.25, py: 0.875, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Box>
                  <Typography sx={{ fontFamily: 'var(--font-mono)', fontSize: '0.625rem', color: 'var(--ink)' }}>
                    {token.note || '—'}
                  </Typography>
                ...
                <Box sx={{ border: `1px solid ${token.redeemed_by_id ? 'rgba(106,154,122,0.4)' : 'rgba(216,207,184,0.2)'}`, borderRadius: '2px', px: 0.75, height: 18, display: 'inline-flex', alignItems: 'center', fontFamily: 'var(--font-mono)', fontSize: '0.625rem', letterSpacing: '0.1em', color: token.redeemed_by_id ? '#6a9a7a' : 'var(--muted)' }}>
